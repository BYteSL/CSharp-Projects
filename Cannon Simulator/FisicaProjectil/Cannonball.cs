...
namespace FisicaProjectil
{
    class Cannonball
    {
        ...
        public double Angulo
        {
            ...
            set
            {
                if (value < 90 && value > 0)
                {
                    ...
                    {
                        angulo = Math.Round(GiveRightAngle() + 1);
                        Cos = angulo;
                        Sen = angulo;
                        Tan = angulo;
                    }
        ...
        #region Metodos
        public bool VerificaY(double y,double angulo,double x,out double valorCorrigido)
        {
            if(y > (Math.Tan(DegToRad(angulo)) * x))
            {
                
                valorCorrigido = Math.Round(((Math.Tan(DegToRad(angulo)) * x) - 1), 0);
                return false;
            }
            else
            {
        ...
        {
            double gravity = Gravidade, tan = 0, time = 0;

            tan = (Tan * X) - Y;

            time = Math.Sqrt(tan / (gravity / 2));
            V0 = Math.Round(X / (time * Cos), 1);
            //V0 = X / (time * Cos);
        ...
        public double Resolvente(double a, double b, double c)
        {
            ...
            var x1 = Math.Round(((-(b)) + delta) / (2.0 * a), 4);
            var x2 = Math.Round(((-(b)) - delta) / (2.0 * a), 4);
